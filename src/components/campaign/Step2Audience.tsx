...
const Step2Audience = ({ value, onChange, onNext, onBack, onError, loading }: Props) => {
  ...
  console.log('Step2Audience - valores actuales:', value);
  ...
  return (
    <div className="max-w-3xl mx-auto w-full py-6 sm:py-8">
      ...
      <div className="flex flex-col sm:flex-row justify-between gap-3 sm:gap-4">
        ...
        <button
          className="w-full sm:w-auto order-1 sm:order-2 mb-3 sm:mb-0 px-5 sm:px-6 py-2.5 sm:py-3 rounded-lg bg-blue-500 text-white font-medium hover:bg-blue-600 transition shadow-sm flex items-center justify-center gap-2 text-sm sm:text-base"
          onClick={() => {
            // IMPORTANTE: Log para diagnóstico
            console.log('Clic en NUEVO botón Continuar - Avanzando inmediatamente');
            console.log('Datos actuales:', value);
            
            // IMPORTANTE: Llamamos a onNext() directamente sin validación
            onNext();
          }}
        >
          {loading ? (
